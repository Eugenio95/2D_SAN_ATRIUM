function CV = CV_in_SEP_vert(t, Vm_mat, x_s1, y_s1, cell_length, nCells)

ind_start = sub2ind([200, 200], x_s1, y_s1);
ind_end   = sub2ind([200, 200], x_s1 + nCells, y_s1);
...
[~, OSpos_start] = findpeaks(diff(Vm_mat(ind_start, :))./diff(t)', 'MinPeakHeight', max(diff(Vm_mat(ind_start, :))./diff(t)')/2,  'MinPeakDistance', 50);
[~, OSpos_end] = findpeaks(diff(Vm_mat(ind_end, :))./diff(t)', 'MinPeakHeight', max(diff(Vm_mat(ind_end, :))./diff(t)')/2, 'MinPeakDistance', 50);

plot(t(OSpos_start), Vm_mat(ind_start, OSpos_start), 'g*')
plot(t(OSpos_end), Vm_mat(ind_end, OSpos_end), 'r*')
